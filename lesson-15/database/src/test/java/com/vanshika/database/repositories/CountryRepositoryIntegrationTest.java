...
public class CountryRepositoryIntegrationTest {
    ...
    @Test
    public void testThatGetCountryWithGdpLessThan(){
        Country countryA=TestDataUtil.createTestCountryA();
        countryRepository.save(countryA);
        Country countryB=TestDataUtil.createTestCountryB();
        countryRepository.save(countryB);
        Country countryC=TestDataUtil.createTestCountryC();
        countryRepository.save(countryC);
        Country countryD=TestDataUtil.createTestCountryD();
        countryRepository.save(countryD);
        Iterable<Country> result=countryRepository.gdpLessThan(13000.00D);
        assertThat(result).containsExactly(countryA,countryC,countryD);

    }
    @Test
    public void testThatGetCountryWithGdpMoreThan(){
        Country countryA=TestDataUtil.createTestCountryA();
        countryRepository.save(countryA);
        Country countryB=TestDataUtil.createTestCountryB();
        countryRepository.save(countryB);
        ...
        Country countryD=TestDataUtil.createTestCountryD();
        countryRepository.save(countryD);
        Iterable<Country> result=countryRepository.findCountryWithGdpMoreThan(13000.00D);
        assertThat(result).containsExactly(countryB);
